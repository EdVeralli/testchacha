...
fn main() {
    let tag_01:GenericArray = chachablake_cipher(b"an example very very secret key.",b"extra long unique nonce!",b"plaintext message" );
}


fn chachablake_cipher(text_key: &'static [u8; 32], text_nonce: &'static [u8; 32], text_assoc: &'static [u8; 32]) -> GenericArray<u8, UInt<UInt<UInt<UInt<UInt<UInt<UTerm, B1>, B0>, B0>  {
